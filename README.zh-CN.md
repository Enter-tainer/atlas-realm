...
- Marker：点位标记。
- Text：地图文字说明。
- Weather：天气卡片。卡片钉在要查询的位置，创建时指定起始日期和天数（默认只查一天）。折叠时就是一个圆点加一行文字：天气图标、最高/最低温、湿度、降水量 mm（多日只显示第一天）。点一下直接展开成完整卡片 —— 卡片头是地名和日期范围（单日折成一行，多日每天一格，逐日的湿度和降水量在悬停提示里），下面嵌入 weather.mgt.moe 的完整预报。缩放低于 7 时只画一个圆点。
- Line：自由路径。
- Area：区域范围。
- Route：带路线信息的路径标注。
