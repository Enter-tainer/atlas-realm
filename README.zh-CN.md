...
Atlas Realm skill 可以用 `npx skills add` 安装到任意 Agent：
...
- Marker：点位标记。
- Text：地图文字说明。
- Weather：天气卡片。卡片钉在要查询的位置，创建时指定起始日期和天数（默认只查一天）。折叠时就是一个圆点加一行文字：天气图标、最高/最低温、湿度、降水量 mm（多日只显示第一天）。点一下直接展开成完整卡片 —— 卡片头是地名和日期范围（单日折成一行，多日每天一格，逐日的湿度和降水量在悬停提示里），下面嵌入 weather.mgt.moe 的完整预报。要查就查一两天，而不是一个月：30 只是输入上限，不是值得拿来规划的地平线。逐日数据来自 Open-Meteo —— 前约 15 天用确定性预报，再远则回退到 `gfs05` 集合预报的成员平均（最远约 35 天），是概率性的，不是官方预报。缩放低于 7 时只画一个圆点。
- Line：自由路径。
- Area：区域范围。
- Route：带路线信息的路径标注。
...
Atlas Realm skill 可以让 Agent 连接到一个房间，读取当前地图状态、检查图层内容，并添加点、路线、区域或说明。Agent 拿到的是房间 URL，而不是一段脱离上下文的 prompt，所以它能在同一份空间文档里留下可见、可继续编辑的结果。
...
packages/atlas-realm-cli/     # Atlas Realm CLI 和 Agent skill
migrations/                  # D1 数据库迁移
